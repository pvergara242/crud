...
crud.post("/api/v1/create/Proveedores", (req, res) => {
...
crud.get("/api/v1/get/Proveedores", (req, res) => {
    Proveedores.find({ is_active: true })
        .then(result => {
            res.status(200).send(result);
        })
        .catch(err => {
...
crud.get("/api/v1/get/Proveedores/:Proveedoresid", (req, res) => {
    Proveedores.findById(req.params.Proveedoresid, (err, response) => {
        !err ? res.status(200).send(response) : res.status(404).send(err);
    });
});

crud.put("/api/v1/update/Proveedores/:Proveedoresid", (req, res) => {
    Proveedores.findByIdAndUpdate(req.params.Proveedoresid, { $set: req.body }, { new: true }, (err, response) => {
        !err ? res.status(200).send(response) : res.status(404).send(err);
    });
});

crud.delete("/api/v1/delete/Proveedores/:Proveedoresid", (req, res) => {
    Proveedores.findByIdAndUpdate(req.params.Proveedoresid, { $set: { is_active: false } }, { new: true }, (err, response) => {
        !err ? res.status(200).send({ message: "proveedor eliminado" }) : res.status(404).send(err);
    });
});
